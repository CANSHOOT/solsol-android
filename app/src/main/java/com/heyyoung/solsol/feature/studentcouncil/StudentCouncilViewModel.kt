...
class StudentCouncilViewModel @Inject constructor(
    ...
    fun loadDeptSummary(month: String? = null, semester: String? = "2025-1") {
        viewModelScope.launch {
            isLoading = true
            runCatching { api.getDeptSummary(month, semester) }
                .onSuccess {
                    summary = it
                    errorMessage = null
                    Log.d("DeptSummary", "result: $summary")
                }
                .onFailure { e ->
                    errorMessage = "홈 요약 실패: ${e.message}"
                    Log.e("StudentCouncilVM", "home failed", e)
                }
            isLoading = false
        }
    }
    ...
    fun loadExpenditures(month: String? = null, page: Int = 0, size: Int = 20) {
        viewModelScope.launch {
            isLoading = true
            runCatching { api.getExpenditures(month = month, page = page, size = size) }
                .onSuccess { resp ->
                    Log.d("StudentCouncilVM", "백엔드 응답: $resp")
                    expenditureList = resp.items.map { item ->
                        CouncilExpenditureResponse(
                            expenditureId = item.expenditureId,
                            councilId = summary?.header?.councilId ?: 0L,
                    ...
                    currentBalance = resp.header.currentBalance.toLong()
                    errorMessage = null
                }
                .onFailure { e ->
    ...
    fun loadFeeStatus(councilId: Long, feeId: Long) {
        viewModelScope.launch {
            isLoading = true
            runCatching { api.getFeeStatus(councilId, feeId) }
                .onSuccess {
                    feeStatus = it
                    errorMessage = null
                    Log.d("FeeStatus", "result: $feeStatus")
                }
                .onFailure { e ->
    ...
    fun addExpenditure(request: CouncilExpenditureRequest) {
        viewModelScope.launch {
            isLoading = true
            runCatching { api.addExpenditure(request) }
                .onSuccess {
    ...
    fun transferFee(request: CouncilFeeTransferCommand, onSuccess: () -> Unit = {}) {
        viewModelScope.launch {
            isLoading = true
            runCatching { api.transferFee(request) }
                .onSuccess {
                    onSuccess()
                    errorMessage = null
                }
