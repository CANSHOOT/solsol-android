...
interface BackendPaymentService {
    ...
    /**
     * QR 스캔 후 결제 정보 조회
     * GET v1/payments/preview/{tempId}
     *
     * QR 스캔 결과와 JWT 토큰을 보내서 상품 정보, 학과별 할인율 등을 조회합니다.
     */
    @GET("payments/preview/{tempId}")
...
data class PaymentPreviewResponse(
    ...
    val coupons: List<DiscountCoupon>? = null,
    val paymentId: Long
) {
    init {
        Log.v("PaymentPreviewResponse", "department=$department")
        Log.v("PaymentPreviewResponse", "paymentId=$paymentId")
        Log.v("PaymentPreviewResponse", "coupons=${coupons?.size ?: 0}개")
    }
}
